...
type Config struct {
	...
	// SCEMemory configures the SCE Memory Service integration for project-level memory injection.
	SCEMemory SCEMemoryConfig `yaml:"sce-memory" json:"sce-memory"`

	// IntentUpgrade configures the optional two-pass intent analysis and prompt upgrade.
	IntentUpgrade IntentUpgradeConfig `yaml:"intent-upgrade" json:"intent-upgrade"`

	legacyMigrationPending bool `yaml:"-" json:"-"`
}

// SCEMemoryConfig configures the embedded SCE Memory query engine.
type SCEMemoryConfig struct {
	// Enable toggles SCE memory integration on or off.
	Enable bool `yaml:"enable" json:"enable"`
	// DBPath is the path to the SCE runtime.db file.
	DBPath string `yaml:"db-path" json:"db-path"`
	// IndexPath is the path to the SCE memory/index.json file.
	IndexPath string `yaml:"index-path" json:"index-path"`
	// MaxResults is the maximum number of SCE hits to retrieve. Default: 8.
	MaxResults int `yaml:"max-results" json:"max-results"`
	// ScoreFloor is the minimum score threshold; hits below this are discarded. Default: 3.
	ScoreFloor int `yaml:"score-floor" json:"score-floor"`
}

// IntentUpgradeConfig configures the two-pass intent analysis and prompt upgrade.
type IntentUpgradeConfig struct {
	// Enable toggles intent upgrade on or off. Default: false.
	Enable bool `yaml:"enable" json:"enable"`
	// ChannelName is the name of a CliRelay channel to use for intent analysis.
	ChannelName string `yaml:"channel-name" json:"channel-name"`
	// Model is the model to use for intent analysis (e.g., "gemini-2.5-flash").
	Model string `yaml:"model" json:"model"`
	// TimeoutMs is the HTTP request timeout for the analysis call. Default: 8000.
	TimeoutMs int `yaml:"timeout-ms" json:"timeout-ms"`
	// MaxInputTokens is the maximum input token count for the analysis prompt. Default: 2000.
	MaxInputTokens int `yaml:"max-input-tokens" json:"max-input-tokens"`
	// APIKeys is an allowlist of API keys that trigger intent upgrade. Empty means all keys.
	APIKeys []string `yaml:"api-keys" json:"api-keys"`
	// ExcludeModels lists models that should never trigger intent upgrade (e.g., the analysis model itself).
