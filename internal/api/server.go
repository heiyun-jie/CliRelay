...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware(), bodyutil.LimitBodyMiddleware(bodyutil.ManagementBodyLimit))
	{
		...
		mgmt.GET("/usage/access-logs", s.mgmt.GetManagementAccessLogs)
		mgmt.GET("/access-logs", s.mgmt.GetManagementAccessLogs)
		mgmt.GET("/usage/chart-data", s.mgmt.GetUsageChartData)
		mgmt.GET("/usage/entity-stats", s.mgmt.GetEntityUsageStats)
		mgmt.GET("/memory", s.mgmt.GetMemoryEntries)
		mgmt.POST("/memory", s.mgmt.PostMemoryEntry)
		mgmt.GET("/memory/applications", s.mgmt.GetMemoryApplications)
		mgmt.GET("/memory/turns", s.mgmt.GetConversationTurns)
