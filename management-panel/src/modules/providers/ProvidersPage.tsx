...
export function ProvidersPage() {
  ...
  const proxyBaseUrl = useMemo(
    () => normalizeApiBase(apiBase || detectApiBaseFromLocation()),
    [apiBase],
  );
  ...
  const resolveProxyClientApiKey = useCallback(async (model: string): Promise<string | null> => {
    const isModelAllowed = (entry: ApiKeyEntry): boolean => {
      const allowedModels = entry["allowed-models"];
      if (!Array.isArray(allowedModels) || allowedModels.length === 0) {
        return true;
      }
      return allowedModels.some(
        (allowedModel) => typeof allowedModel === "string" && allowedModel.trim() === model,
      );
    };

    try {
      const entries = await apiKeyEntriesApi.list();
      const enabledEntries = entries.filter((entry) => entry?.key?.trim() && !entry.disabled);
      const matchedEntry = enabledEntries.find(isModelAllowed) ?? enabledEntries[0];
      if (matchedEntry?.key?.trim()) {
        return matchedEntry.key.trim();
      }
    } catch {
      // ignore and fallback to legacy api-keys
    }

    try {
      const legacyKeys = await apiKeysApi.list();
      const fallbackKey = legacyKeys.find((key) => key.trim());
      if (fallbackKey) {
  ...
  // Shared: test a model through the local CliRelay proxy via backend /api-call
  const testModelViaProxy = useCallback(async (model: string): Promise<{ ok: boolean; msg: string }> => {
    try {
      const clientApiKey = await resolveProxyClientApiKey(model);
      if (!clientApiKey) {
        return { ok: false, msg: "✗ 请先在 API Keys 页面创建可用客户端 Key" };
      }
      ...
      const result = await apiCallApi.request({
        ...
        data: JSON.stringify({ model, messages: [{ role: "user", content: "hi" }], max_tokens: 5, stream: false }),
      });
      if (result.statusCode >= 200 && result.statusCode < 300) {
        const content = (result.body as any)?.choices?.[0]?.message?.content || "";
        return { ok: true, msg: `✓ ${content.slice(0, 40) || "连通"}` };
      }
      const raw = getApiCallErrorMessage(result);
      const msg = raw.length > 100 || raw.includes("<") ? `HTTP ${result.statusCode}` : raw;
      return { ok: false, msg: `✗ ${msg}` };
    } catch (err) {
      return { ok: false, msg: `✗ ${err instanceof Error ? err.message : "测试失败"}` };
    }
  }, [proxyBaseUrl, resolveProxyClientApiKey]);

  // Key card test: pick first model from the key config
  const handleKeyTest = useCallback(
    (items: ProviderSimpleConfig[]) => async (idx: number) => {
      const model = items[idx]?.models?.[0]?.name;
      if (!model) return { ok: false, msg: "✗ 未配置模型" };
      return testModelViaProxy(model);
    },
    [testModelViaProxy],
  );

  // OpenAI provider connectivity test
  const [openaiTestingIdx, setOpenaiTestingIdx] = useState<number | null>(null);
  const [openaiTestResults, setOpenaiTestResults] = useState<Record<number, { ok: boolean; msg: string }>>({});

  const handleOpenaiTest = useCallback(async (provider: OpenAIProvider, idx: number) => {
    setOpenaiTestingIdx(idx);
    setOpenaiTestResults((prev) => ({ ...prev, [idx]: undefined! }));
    try {
      const model = provider.models?.[0]?.name;
      if (!model) {
        setOpenaiTestResults((prev) => ({ ...prev, [idx]: { ok: false, msg: "✗ 未配置模型" } }));
        return;
      }
      const result = await testModelViaProxy(model);
      setOpenaiTestResults((prev) => ({ ...prev, [idx]: result }));
    } finally {
      setOpenaiTestingIdx(null);
    }
  }, [testModelViaProxy]);
      ...
      <Tabs value={tab} onValueChange={(next) => {
        ...
        <TabsContent value="gemini" className="mt-6">
          <ProviderKeyListCard
            ...
            onTest={handleKeyTest(geminiKeys)}
          />
        </TabsContent>
        ...
        <TabsContent value="claude" className="mt-6">
          <ProviderKeyListCard
            ...
            getStats={getSimpleStats}
            getStatusBar={getSimpleStatusBar}
            onTest={handleKeyTest(claudeKeys)}
          />
        </TabsContent>
        ...
        <TabsContent value="codex" className="mt-6">
          <ProviderKeyListCard
            ...
            getStats={getSimpleStats}
            getStatusBar={getSimpleStatusBar}
            onTest={handleKeyTest(codexKeys)}
          />
        </TabsContent>
        ...
        <TabsContent value="vertex" className="mt-6">
          <ProviderKeyListCard
            ...
            getStats={getSimpleStats}
            getStatusBar={getSimpleStatusBar}
            onTest={handleKeyTest(vertexKeys)}
          />
        </TabsContent>
                ...
                {openaiProviders.map((provider, idx) => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            variant="secondary"
                            size="sm"
                            disabled={openaiTestingIdx === idx}
                            onClick={() => handleOpenaiTest(provider, idx)}
                          >
                            {openaiTestingIdx === idx ? <Loader2 size={14} className="animate-spin" /> : <Play size={14} />}
                            测试
                          </Button>
                          <Button
                            variant="secondary"
                      ...
                      {openaiTestResults[idx] ? (
                        <p className={`mt-1 text-xs font-medium ${openaiTestResults[idx].ok ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
                          {openaiTestResults[idx].msg}
                        </p>
                      ) : null}
                    </div>
                  );
